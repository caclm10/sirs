    ...
    <div class="d-flex h-100 container">
        <main class="m-auto card px-5 py-4 w-100 shadow-lg" style="max-width:450px;">
            ...
            <div class="text-center mb-4">
                ...
                <h1 class="fs-4 text-uppercase" style="font-weight:400">{{ config('custom.app_name') }}</h1>
            </div>

            <form
                action="@if ($admin) {{ route('authenticate.admin') }} @else {{ route('authenticate') }} @endif"
                method="POST" class="mb-5">
                @csrf
                @if ($admin)
                    <div class="form-floating mb-3 relative">
                        <span class="iconify" data-icon="carbon:identification"></span>
                        <input type="text" class="form-control ps-5 @error('kode') is-invalid @enderror"
                            name="kode" id="kode" placeholder="Kode" value="{{ old('kode') }}">
                        <label for="kode" class="form-label" style="padding-left: 55px;">Kode</label>
                        @error('kode')
                ...
                @else
                    <div class="form-floating mb-3 relative">
                        <span class="iconify" data-icon="carbon:identification"></span>
                        <input type="text" class="form-control ps-5 @error('nomor') is-invalid @enderror"
                            name="nomor" id="nomor" placeholder="NIS / NIP" value="{{ old('nomor') }}">
